...
void create_ui(void* phy_fb, int screen_width, int screen_height, int color_bytes, struct EXTERNAL_GFX_OP* gfx_op) {
	...
	while(1) {
		theCube.draw(64, 30, true);//erase footprint
		theCube.rotate();
		theCube.draw(64, 30, false);//refresh cube

		thePyramid.draw(64, 20, true);//erase footprint
		thePyramid.rotate();
		thePyramid.draw(64, 20, false);//refresh pyramid
		OLED_Refresh();
		thread_sleep(5);
	}
}
